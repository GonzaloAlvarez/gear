...
- `kauket`: installs the PIE `android_arm64` artifact (kauket >= 2.2.2). The
  ...
  kauket also ships an argv shim for system_linker_exec, a DNS fallback
  resolver (no `/etc/resolv.conf` on Android; override with `KAUKET_DNS`),
  and spawns subprocesses through linker64 (raw Go execve of app-data
  binaries is blocked, so without it `gh` auth silently fell back to the
  device flow). Note Docker containers mask all of these — only a real
  device proves them.
